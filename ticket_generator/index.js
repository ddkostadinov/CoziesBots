...
client.on("interactionCreate", async (click) => {
  if (click.isSelectMenu()) {
    ...
    if (click.customId == "selectbg") {
      ...
      if (value === "first_option") {
        await click.reply({
          embeds: [components.blueEmbed],
          components: [components.sky_blue_button],
          ephemeral: true,
        });
      } else if (value === "second_option") {
        await click.reply({
          embeds: [components.yellowEmbed],
          components: [components.yellow_button],
          ephemeral: true,
        });
      } else if (value === "third_option") {
        await click.reply({
          embeds: [components.pinkEmbed],
          components: [components.pink_button],
          ephemeral: true,
        });
      } else if (value === "fourth_option") {
        await click.reply({
          embeds: [components.purpleEmbed],
          components: [components.purple_button],
          ephemeral: true,
        });
      } else if (value === "fifth_option") {
        await click.reply({
          embeds: [components.oceanBlueEmbed],
          components: [components.ocean_blue_button],
          ephemeral: true,
        });
      }
    }
    ...
    if (click.customId == "main_button") {
      ...
      for (let i = 0; i < questRole.length; i++) {
        if (passenger.has(questRole[i])) {
          ...
          await click.reply({
            content:
              "***Only an Adventurer or above can interact with the machine***",
            ephemeral: [true],
          });
        }
      }
  ...
  if (click.customId === "myModal") {
    ...
    click.reply({
      content: `*The ticket machine makes a few beep-boop sounds and produces your ticket.*`,
      ephemeral: true,
      files: [img],
    });
  }
